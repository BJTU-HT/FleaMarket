...
@interface SecondhandFilterView () <UITableViewDataSource, UITableViewDelegate>
...
@implementation SecondhandFilterView
...
{
    // 解析JSON
    NSString *strPath = [[NSBundle mainBundle] pathForResource:@"SchoolJSON" ofType:@"json"];
    ...
    for (int i = 0; i < dataArray1.count; i++) {
        SchoolGroupModel *schoolM = [SchoolGroupModel objectWithKeyValues:dataArray1[i]];
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if (tableView.tag == 10) {
        ...
        SchoolGroupModel *cateM = (SchoolGroupModel *)_bigGroupArray[_bigSelectedIndex];
        if (cateM.list == nil) {
            return 0;
        }else{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView.tag == 10) {
        ...
        SecondhandFilterCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIndentifier];
        if (cell == nil) {
            cell = [[SecondhandFilterCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIndentifier withFrame:CGRectMake(0, 0, screenWidthPCH/2, 42)];
        }
        
        SchoolGroupModel *cateM = _bigGroupArray[indexPath.row];
