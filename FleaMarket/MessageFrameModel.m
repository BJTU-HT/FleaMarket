...
+ (NSMutableArray *)frameModelWithArray:(NSMutableArray *)arr
{
    ...
    for (SecondhandMessageVO *model in arr) {
        MessageFrameModel *frameModel = [self frameModelWithModel:model];
...
- (CGFloat)cellHeight
{
    if (_cellHeight == 0) {
        ...
        // 发布时间
        NSDictionary *publishTimeAttrs = @{NSFontAttributeName : HTNameFont};
        ...
        self.messageFrame = CGRectMake(messageX, messageY, messageW,
                                       txtFrame.size.height =[_model.content boundingRectWithSize:
                                                              CGSizeMake(txtFrame.size.width, CGFLOAT_MAX)
                                                                                       ...
                                                                                       attributes:[NSDictionary dictionaryWithObjectsAndKeys:HTTextFontLess,NSFontAttributeName, nil] context:nil].size.height);
